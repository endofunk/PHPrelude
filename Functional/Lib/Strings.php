...
class Strings extends Module {
	 ...
	 * @return string
	 */
	protected static function __lchomp(string $toChomp, string $str): string{
		$length = strlen($toChomp);
		if (strcmp(substr($str, 0, $length), $toChomp) === 0) {
	 ...
	 * @return string
	 */
	protected static function __rchomp(string $toChomp, string $str): string{
		$length = strlen($toChomp);
		if (strcmp(substr($str, -$length, $length), $toChomp) === 0) {
	 ...
	 * @return string
	 */
	protected static function __chomp(string $toChomp, string $str): string{
	 ...
	 */
	protected static function __join(string $on, array $arrstr): string {
		return implode($on, $arrstr);
	}
	...
	protected static function __vardump($var): string{
		ob_start();
		var_dump($var);
		$result = ob_get_clean();
		return $result;
	...
	protected static function __log(string $title, $value): string {
		return str_repeat("-", 70 - strlen($title)) . " " . $title . " " . str_repeat("-", 4) . "\n" . Strings::vardump($value);
	}
}
